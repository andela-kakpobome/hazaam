...
exports.debugQuery = (req, res) => {
  ...
  fingerprinter.decodeCodeString(code, function(err, fp) {
    ...
    (err, result, allMatches) => {
      ...
      function renderView() {
        const json = JSON.stringify({ success: !!result.success, status: result.status,
...
function getContributors(fp, match) {
  ...
  const keys = Object.keys(match.histogram);
  const array = new Array(keys.length);
  for (i = 0; i < keys.length; i++)
    array[i] = [ parseInt(keys[i], 10), match.histogram[keys[i]] ];
  array.sort(function(a, b) { return b[1] - a[1]; });
  const topOffsets = array.splice(0, 2);

  const matchCodesToTimes = fingerprinter.getCodesToTimes(match, fingerprinter.MATCH_SLOP);

  // Iterate over each {code,time} tuple in the query
  for (i = 0; i < fp.codes.length; i++) {
    const code = fp.codes[i];
    const time = Math.floor(fp.times[i] / fingerprinter.MATCH_SLOP) * fingerprinter.MATCH_SLOP;
